...
class Command(LogBaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '-p', '--http-addr', type=int, default=8888, help='Port used by the web UI'
        )
        parser.add_argument(
            '-j',
            '--join',
            action='append',
            help='Initial agent(s) to join with (can be used multiple times)',
        )
        parser.add_argument(
            '-e',
            '--encrypt',
    ...
    def handle(self, *_, **options):
        ...
        if (
            ...
            and settings.DKRON_TOKEN
            and settings.SENTRY_CRON_URL
        ):
            flag_name = (
                '--pre-webhook-url'
                if utils.dkron_binary_version() < (3, 2, 0)
                else '--pre-webhook-endpoint'
            )
            args.extend([
                flag_name,
                settings.DKRON_PRE_WEBHOOK_URL,
